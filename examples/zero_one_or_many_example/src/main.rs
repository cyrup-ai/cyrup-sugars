...
impl EventBus {
    ...
    fn add_listener(&mut self, event: String, listener: String) {
        let current = self.listeners.remove(&event).unwrap_or(ZeroOneOrMany::none());
        self.listeners.insert(event, current.with_pushed(listener));
    }
    
    fn get_listeners(&self, event: &str) -> ZeroOneOrMany<String> {
        self.listeners.get(event).cloned().unwrap_or(ZeroOneOrMany::none())
    }
...
fn builder_pattern_example() {
    ...
    println!("  Server 2 middleware count: {}", server2.middleware_count());
}
...
fn event_handling_example() {
    ...
    println!("  user_login listeners: {}", bus.listener_count("user_login"));
    println!("  user_logout listeners: {}", bus.listener_count("user_logout"));
    println!("  user_signup listeners: {}", bus.listener_count("user_signup"));
...
fn safe_access_example() {
    ...
    for (i, collection) in collections.iter().enumerate() {
        ...
        println!("    Length: {}, Empty: {}", collection.len(), collection.is_empty());
        println!();
    }
}